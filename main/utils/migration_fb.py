...
def season_map(old_db, model):
    return model_map(old_db, 'main_season', model, ('id', 'season'), ('season', ))


def club_map(old_db, model):
    return model_map(old_db, 'main_club', model, ('id', 'name'), ('name', ))


def ground_map(old_db, model):
    return model_map(old_db, 'main_ground', model, ('name', ), ('name', ))


def round_map(old_db, model):
    return model_map(
        old_db, 'main_round', model,
        ('id', 'season_id', 'name'),
        ('season__season', 'name')
    )
...
def tip_map(old_db, model, fk_models):
    ...
    return model_map(
        old_db, 'main_tip', model,
        ('id', ('game_id', g_map), ('club_id', c_map)),
        ('game', 'club')
    )
