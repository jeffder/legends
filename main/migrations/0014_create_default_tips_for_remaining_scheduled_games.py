...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Create default tips for remaining scheduled games so that every coach
        has a default tip for every remaining game.
        """
        season = orm.Season.objects.get(season=2014)
        coaches = season.coaches.all()
        all_clubs = set(c.club for c in coaches)
        scheduled = orm.Game.objects \
            .filter(round__season=season) \
            .filter(status='Scheduled') \
            .order_by('round')

        new_tips = []
        for rnd, games in itertools.groupby(scheduled, lambda x: x.round):
            if rnd.is_finals:
                tipping_clubs = set(
                    c.club for c in coaches if c.can_tip_in_round(self)
                )
            else:
                tipping_clubs = all_clubs

            for game in games:
                tips = game.tips.all()

                # If there are tips missing, create default tips for the missing
                # clubs
                if len(tips) < len(tipping_clubs):
                    tipped = {t.club for t in tips}
                    missing = tipping_clubs - tipped
                    for miss in missing:
                        tip = orm.Tip(club=miss, game=game, is_default=True)
                        new_tips.append(tip)

        orm.Tip.objects.bulk_create(new_tips)

    def backwards(self, orm):
        """
        Just remove all default tips for scheduled games.
        """
        orm.Tip.objects \
            .filter(game__round__season__season=2014) \
            .filter(game__status='Scheduled') \
