...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        games = orm.Game.objects.all()
        for game in games:
            sc_rankings = {
                r.player: r.ranking for r in game.supercoach_rankings.all()
            }
            season = game.round.season.season
            if season <= 2011:
                points = 3
            else:
                points = 2

            for sc_tip in orm.SupercoachTip.objects.filter(tip__game=game):
                try:
                    sc_tip.score = sc_rankings[sc_tip.player] * points
                except KeyError:
                    sc_tip.score = 0
                sc_tip.save()
