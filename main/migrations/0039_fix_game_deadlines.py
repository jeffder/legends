...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        rounds = orm.Round.objects.filter(season__season=2015)

        for rnd in rounds:
            _deadline = None
            _day = None
            for game in rnd.games.all():
                day = game.game_date.weekday()
                if day == _day:
                    game.tipping_deadline = _deadline
                else:
                    _day = day
                    # Set deadline for weekend games if day is Friday
                    # Allow for rounds with no Friday game
                    if day == 4 or (rnd.name == 'Round 1' and day == 5):
                        _deadline = game.game_date - delta
    ...
    models = {
        ...
        'main.pastcategorywinner': {
            'Meta': {'db_table': "'main_past_category_winner'", 'ordering': "('-season__season', 'category', 'club')", 'object_name': 'PastCategoryWinner'},
        ...
        'main.pastcoach': {
            'Meta': {'db_table': "'main_past_coach'", 'ordering': "['-season', 'club', 'last_name', 'first_name']", 'object_name': 'PastCoach'},
        ...
        'main.streakladder': {
            'Meta': {'db_table': "'main_streak_ladder'", 'ordering': "['wins', 'draws', '-losses', 'club']", 'object_name': 'StreakLadder'},
        ...
        'main.supercoachtip': {
            'Meta': {'db_table': "'main_supercoach_tip'", 'ordering': "('player__last_name', 'player__initial', 'player__first_name')", 'object_name': 'SupercoachTip'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
